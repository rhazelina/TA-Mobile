...
class DetailJadwalGuruActivity : AppCompatActivity() {
    ...
    // Gunakan konstanta dari AbsensiSiswaActivity atau buat sendiri
    companion object {
    ...
    private val qrScannerLauncher = registerForActivityResult(
        ...
        if (result.resultCode == RESULT_OK) {
            ...
            // Perbaikan: Gunakan konstanta dari CameraQRActivity
            val isSuccess = data?.getBooleanExtra(CameraQRActivity.EXTRA_QR_RESULT, false) ?: false

            if (isSuccess) {
                // Untuk mendapatkan data dari scan QR, kita perlu memodifikasi CameraQRActivity
                // atau mendapatkan data dari jadwal yang sedang aktif
                val kelas = currentJadwal.kelas
                val mapel = currentJadwal.mataPelajaran

                // Buat tanggal dan jam saat ini
                val sdf = SimpleDateFormat("dd-MM-yyyy HH:mm", Locale.getDefault())
                val tanggalWaktu = sdf.format(Date())
                ...
                val intent = Intent(this, AbsensiSiswaActivity::class.java).apply {
                    // Gunakan konstanta yang didefinisikan di sini atau dari AbsensiSiswaActivity
                    putExtra(EXTRA_MAPEL, mapel)
                    putExtra(EXTRA_KELAS, kelas)
                    putExtra(EXTRA_TANGGAL, SimpleDateFormat("dd-MM-yyyy", Locale.getDefault()).format(Date()))
                    putExtra(EXTRA_JAM, currentJadwal.jam)
                }
