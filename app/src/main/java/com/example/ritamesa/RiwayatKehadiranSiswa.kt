...
class RiwayatKehadiranSiswa : AppCompatActivity() {
    ...
    private val semuaKehadiran = listOf(
        ...
        KehadiranItem(10, "Eka Putri", "Siswa", "alpha", "07:00",
            "Senin, 7 Januari 2026", "-", "Alpha")
    ) // TUTUP LIST DENGAN TANDA KURUNG DAN KURUNG KUrawal TUTUP
                    ...
                    if (response.isSuccessful && response.body() != null) {
                        ...
                        val filtered = if (currentStatus != "Semua") {
                            dataKehadiran.filter {
                                it.statusDetail.equals(currentStatus, true) ||
                                        it.status.equals(currentStatus, true)
                            }
                        } else {
                            dataKehadiran
                        }
                        ...
                        // Apply role filter
                        val finalFiltered = if (currentRole != "Semua") {
                            filtered.filter { it.role.equals(currentRole, true) }
                        } else {
                            filtered
                        }

                        recyclerView.adapter = KehadiranAdapter(finalFiltered)
                        updateStatistics(finalFiltered)
    ...
    private fun loadDummyData() {
        dataKehadiran.clear()
        dataKehadiran.addAll(semuaKehadiran)
    ...
    private fun updateStatistics(filteredList: List<KehadiranItem>) {
        ...
        val hadir = filteredList.count {
            it.status.equals("hadir", true) ||
                    it.status.equals("present", true) ||
                    it.statusDetail.contains("hadir", true) ||
                    it.statusDetail.contains("tepat", true)
        }
        val terlambat = filteredList.count {
            it.status.equals("terlambat", true) ||
                    it.status.equals("late", true) ||
                    it.statusDetail.contains("terlambat", true)
        }
        val izin = filteredList.count {
            it.status.equals("izin", true) ||
                    it.status.equals("excused", true) ||
                    it.statusDetail.contains("izin", true)
        }
        val sakit = filteredList.count {
            it.status.equals("sakit", true) ||
                    it.status.equals("sick", true) ||
                    it.statusDetail.contains("sakit", true)
        }
        val alpha = filteredList.count {
            it.status.equals("alpha", true) ||
                    it.status.equals("absent", true) ||
                    it.statusDetail.contains("alpha", true)
        }
